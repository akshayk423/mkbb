                ...
                <div class="child">
                    <form id="filterOptions" action="stabs.jsp" method="post">
                        ...
                        <div id="sizeCheckbox" class="form-check form-check-inline">
                            ...
                            <label class="form-check-label">1U</label>

                            <input class="form-check-input" type="checkbox"  value="2U" name="size2u">
                            <label class="form-check-label">2U</label>

                            <input class="form-check-input" type="checkbox" value="2.25U" name="size2.25u">
                            <label class="form-check-label">2.25U</label>

                            <input class="form-check-input" type="checkbox"  value="2.75U" name="size2.75u">
                            <label class="form-check-label">2.75U</label>

                            <input class="form-check-input" type="checkbox"  value="6.25U" name="size6.25u">
                            <label class="form-check-label">6.25U</label>

                            <input class="form-check-input" type="checkbox"  value="7U" name="size7u">
                            <label class="form-check-label">7U</label>
                        ...
                        <div id="mountCheckbox">
                            <text>Mount:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</text>
                            
                            <label>
                                <input class="form-check-input" type="checkbox"value="plate mount" name="plate">Plate
                            </label>
                           
                            
                            <label>
                                <input class="form-check-input" type="checkbox"value="pcb mount" name="pcb">PCB
                            </label>
                        ...
                        <div id="typeCheckbox">
                            <text>Type:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</text>
                            <label>
                                <input class="form-check-input" type="checkbox"value="screw-in" name="screwIn">Screw-In
                            </label>

                            <label>
                                <input class="form-check-input" type="checkbox"value="snap-in" name="snapIn">Snap-In
                            </label>
                        </div>
        ...
        <table id="casesListings" width="100%">
            ...
            <tbody id="rows">
                <%
                    ...
                    String query = "SELECT * FROM mkdb.keyboardpart NATURAL JOIN mkdb.stabilizers;";
                    String filter = request.getParameter("filter");
                    String sort = request.getParameter("sort");
                    String partListID = "";
                    String addStabilzer = request.getParameter("addStabilzer");
                    if(addStabilzer != null && addStabilzer.equals("Add Stabilizer")){
                    ...
                    if(filter != null && filter.equals("filter")){
                        String size1u = request.getParameter("size1u");
                        String size2u = request.getParameter("size2u");
                        String size225u = request.getParameter("size2.25u");
                        String size275u = request.getParameter("size2.75u");
                        String size625u = request.getParameter("size6.25u");
                        String size7u = request.getParameter("size7u");
                        String pcb = request.getParameter("pcb");
                        String plate = request.getParameter("plate");
                        String screwIn = request.getParameter("screwIn");
                        String snapIn = request.getParameter("snapIn");

                        ArrayList<String> filters = new ArrayList<>();
                        ArrayList<String> subFilters = new ArrayList<>();

                        if(size1u != null){filters.add(size1u);}
                        if(size2u != null){filters.add(size2u);}
                        if(size225u != null){filters.add(size225u);}
                        if(size275u != null){filters.add(size275u);}
                        if(size625u != null){filters.add(size625u);}
                        if(size7u != null){filters.add(size7u);}

                        if(pcb != null){subFilters.add(pcb);}
                        if(plate != null){subFilters.add(plate);}
                        if(screwIn != null){subFilters.add(screwIn);}
                        if(snapIn != null){subFilters.add(snapIn);}

                        if(filters.size() > 0){
                            query = query.substring(0,query.length()-1) + " WHERE ";
                            Iterator iterator = filters.iterator();
                            query += "stabType LIKE '%" + iterator.next() + "%'";
                            while(iterator.hasNext()){
                                query += " OR stabType LIKE '%" + iterator.next() + "%'";
                            }

                            if(subFilters.size() > 0){
                                Iterator subIt = subFilters.iterator();
                                while(subIt.hasNext()){
                                    query += " OR info LIKE '%" + subIt.next() + "%'";
                                }
                            }
                            query += ";";
                        }
                    ...
                    while(rs.next()){
                        ...
                        %><tr>
                            ...
                            <td width="10%"><%=rs.getString("stabtype")%></td>
                            <td width="10%"><%=rs.getString("info")%></td>
                            <td>
